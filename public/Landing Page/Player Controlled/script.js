    ...
    function checkCollision() {
        ...
        for (let i = 0; i < redBalls.length; i++) {
            ...
            if (dist < ballRadius + redRadius) {
                ...
                // --- Add this block for red ball spawning ---
                if (ballsEaten % 10 === 0 && redBalls.length < 25) {
                    let toAdd = Math.min(5, 25 - redBalls.length);
        ...
        if (powerUpActive) {
            ...
            for (let i = redBalls.length - 1; i >= 0; i--) {
                ...
                if (dist < magnetRadius) {
                    ...
                    // Red ball spawn logic (every 10 balls)
                    if (ballsEaten % 10 === 0 && redBalls.length < 25) {
                        let toAdd = Math.min(5, 25 - redBalls.length);
                        for (let j = 0; j < toAdd; j++) {
                            redBalls.push(spawnRedBall());
